...
describe("PatientController", () => {
  ...
  test("case method: registerPatient - completed successfully", async () => {
    ...
    mockService.createPatient.mockResolvedValueOnce(new Patient({ name: "Alice Silva", cpf: "74292807060" }));
  ...
  test("case method: registerPatient - Not completed. Serivce Error: Patient already in Database", async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ error: "Já existe um paciente com este CPF cadastrado." }));
  });
  ...
  test("case method: editPatient - Completed successfully", async () => {
    ...
    mockService.updatePatient.mockResolvedValueOnce(new Patient({ name: "Novo nome", cpf: "39133787077" }));
  ...
  test("case method: editPatient - Service Error: No patient found", async () => {
    ...
    //Expected Answers
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ error: "Paciente não encontrado." }));
  });
  ...
  test("case method: inactivatePatient - Completed Successfully", async () => {
    ...
    // Expected answers
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ message: "Paciente inativado com sucesso." }));
  });
  ...
  test("case method: activatePatient - Completed Successfully", async () => {
    ...
    //Expected Answers
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ message: "Paciente ativado com sucesso." }));
  });

  test("case method: getPatientByID - returns patiente data", async () => {
    const { req, res } = mockReqRes();
    req.params = { id: "7" };
    mockService.getPatientById.mockResolvedValueOnce(new Patient({ id: 7, name: "Joao com ID" }));
  ...
  test("case method: getPatientById - Service Errir: Generic internal error", async () => {
    ...
    //Expected Answers
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ error: "Erro ao buscar paciente." }));
  });
  ...
  test("case method: listPatients - Service Errir: Generic internal error", async () => {
      const { req, res } = mockReqRes();
      mockService.getAllPatients.mockRejectedValueOnce(new Error("Erro ao listar pacientes."));
      await patientController.listPatients(req, res);

      //Expected Answers
      expect(res.status).toHaveBeenCalledWith(400);
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ error: "Erro ao listar pacientes." }));
  });
});
